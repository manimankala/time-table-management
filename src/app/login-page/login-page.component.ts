...
export class LoginPageComponent {
  ...
  getProfileData() {
    this.timeTable.getProfileData({ userId: localStorage.getItem('userId') })?.subscribe({
      next: (res: any) => {
        this.profileData = res.data[0]
        localStorage.setItem('profileData', JSON.stringify(this.profileData)); // Store profile data in localStorage
      },
      error: (err) => {
        this.timeTable.snackBarOpen("Error Please Try Again", "ok")
      }
    }
    )
  }
